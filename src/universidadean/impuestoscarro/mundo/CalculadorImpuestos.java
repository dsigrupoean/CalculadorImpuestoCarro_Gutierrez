...
public class CalculadorImpuestos {
    ...
    private RangoImpuesto buscarRangoImpuesto(double valor) {
        ...
        for (int i = 0; i < (rangosImpuesto.length - 1); i++){
    ...
    public double calcularPago(boolean descProntoPago, boolean descServicioPublico, boolean descTrasladoCuenta) {
        ...
        if(descProntoPago) {
            pago -= (pago * PORC_DESC_PRONTO_PAGO ) / 100; //descuento prointo pago
        }
        if(descServicioPublico){
            pago -= VALOR_DESC_SERVICIO_PUBLICO; //descuento por servicio público
        }
        if(descTrasladoCuenta){
            pago -= (pago * PORC_DESC_TRASLADO_CUENTA) / 100; //descuento por traslado de cuenta
        }
    ...
    public Vehiculo buscarVehiculoMasAntiguo() {
        ...
        for(Vehiculo v: vehiculos) {
            if (Integer.parseInt(buscado.darAnio()) > Integer.parseInt(v.darAnio())) {
                buscado = v;
            }
        }
